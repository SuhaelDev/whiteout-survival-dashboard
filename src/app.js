...
const ASSET_CACHE_VERSION = "20260731f";
...
function renderCharms() {
  ...
  $("#tab-charms").innerHTML = `
    <div class="toolbar"><div><h2>Chief Charms</h2><p>Grouped by the troop type of the gear piece each charm is attached to. One level takes four upgrade taps in game, or five from level 12 up, so these costs are several times what a single tap quotes you.</p></div>${charmBulk}</div>
    <div class="chief-gear-layout-container">
      <div class="chief-gear-screen">
        <div class="chief-gear-grid-container">
          <div class="chief-gear-col-left">
...
function hero3dOutfitPiece(T, kind, materials) {
  ...
  if (kind === "hoodedHead") {
    ...
    [-1, 1].forEach((side) => {
      ...
      brow.position.set(side * 0.058, 0.09, 0.182);
      brow.rotation.z = side * -0.15;
      group.add(brow);
    });
    const nose = new T.Mesh(new T.SphereGeometry(0.024, 10, 8), skin);
    nose.position.set(0, 0.015, 0.2);
    group.add(nose);
    const beard = new T.Mesh(new T.SphereGeometry(0.095, 16, 12), fur);
    beard.scale.set(1.35, 1.15, 0.75);
    beard.position.set(0, -0.08, 0.115);
    group.add(beard);
    const mustache = new T.Mesh(new T.SphereGeometry(0.05, 12, 8), fur);
    mustache.scale.set(1.5, 0.5, 0.65);
    mustache.position.set(0, -0.03, 0.185);
    group.add(mustache);
  } else if (kind === "collar") {
